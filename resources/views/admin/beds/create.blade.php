...
<div class="mb-6">
    ...
    <p class="text-gray-600 mt-1">Create a new bed in a room</p>
</div>
...
<div class="bg-white rounded-lg shadow p-8 max-w-2xl">
    <form action="{{ route('admin.beds.store') }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="bed_number" value="{{ old('bed_number') }}" class="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-teal-500 @error('bed_number') border-red-500 @enderror" placeholder="e.g. 101-A, 101-B" required>
            @error('bed_number')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="bed_type" class="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-teal-500 @error('bed_type') border-red-500 @enderror" required>
                ...
                <option value="Standard" {{ old('bed_type') == 'Standard' ? 'selected' : '' }}>Standard</option>
                <option value="Premium" {{ old('bed_type') == 'Premium' ? 'selected' : '' }}>Premium</option>
                <option value="Deluxe" {{ old('bed_type') == 'Deluxe' ? 'selected' : '' }}>Deluxe</option>
            </select>
            @error('bed_type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
