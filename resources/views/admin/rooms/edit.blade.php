...
<div class="bg-white rounded-lg shadow p-8 max-w-2xl">
    <form action="{{ route('admin.rooms.update', $room->id) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="room_number" value="{{ old('room_number', $room->room_number) }}" class="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-teal-500 @error('room_number') border-red-500 @enderror" required>
            @error('room_number')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="number" name="bed_capacity" value="{{ old('bed_capacity', $room->bed_capacity) }}" min="1" class="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-teal-500 @error('bed_capacity') border-red-500 @enderror" required>
            @error('bed_capacity')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
